                    ...
                    if (mat < 10032) {
                        if (mat < 10016) {
                            if (mat < 10008) {
                                if (mat < 10004) { // No directional shading
                                ...
                                else /*if (mat < 10008)*/ { // Grounded Waving Foliage (includes flowers)
                                    ...
                                    // Make only flowers glow, not grass or green plants
                                    float luminance = GetLuminance(color.rgb);
                                    float greenDominance = color.g - max(color.r, color.b);
                                    // Balanced conditions to exclude grass while keeping flowers
                                    if (luminance > 0.3 && 
                                        (greenDominance < 0.0 || (luminance > 0.5 && greenDominance < 0.05)) && 
                                        (color.r > 0.4 || color.b > 0.4) && 
                                        (color.r > color.g * 1.05 || color.b > color.g * 1.05 || luminance > 0.6)) { // Allow very bright flowers even with some green
                                        // Calculate flower glow based on distance from center and color intensity
                                        float centerDistance = length(signMidCoordPos);
                                        float glowRadius = 0.8; // Larger glow radius
                                        float glowFalloff = max(0.0, 1.0 - centerDistance / glowRadius);
                                        emission = luminance * 3.0 * (0.4 + 0.6 * glowFalloff); // More intense glow
                                        color.rgb *= 1.1 + 0.3 * glowFalloff; // Enhanced brightness
                                    }
                                }
                            } else {
                                if (mat < 10012) { // Leaves
                            ...
                            if (mat < 10024) {
                                if (mat < 10020) { // Non-waving Foliage (includes spore_blossom, pink_petals)
                                    subsurfaceMode = 1, noSmoothLighting = true, noDirectionalShading = true;
                                    
                                    // Make only flowers glow, not grass, seagrass or vines
                                    float luminance = GetLuminance(color.rgb);
                                    float greenDominance = color.g - max(color.r, color.b);
                                    // Balanced conditions to exclude grass while keeping flowers
                                    if (luminance > 0.3 && 
                                ...
                                else /*if (mat < 10024)*/ { // Upper Waving Foliage (includes tall flowers upper part)
                                    subsurfaceMode = 1, noSmoothLighting = true, noDirectionalShading = true;
                                    ...
                                    // Make only tall flowers glow, not tall grass
                                    float luminance = GetLuminance(color.rgb);
                                    float greenDominance = color.g - max(color.r, color.b);
                                    // Balanced conditions to exclude grass while keeping flowers
                                    if (luminance > 0.3 && 
                    ...
                    if (mat < 10736) {
                        if (mat < 10720) {
                            ...
                            if (mat < 10728) {
                                ...
                                if (mat < 10732) { // Flower Pot, Potted Stuff:Without Subsurface
                                ...
                                else /*if (mat < 10736)*/ { // Potted Stuff:With Subsurface (includes potted flowers)
                                    noSmoothLighting = true;
                                    ...
                                    if (abs(abs(NdotE) - 0.5) < 0.4) {
                                        ...
                                        // Make potted flowers glow with improved coverage
                                        float luminance = GetLuminance(color.rgb);
                                        float greenDominance = color.g - max(color.r, color.b);
                                        // Better conditions for potted flowers
                                        if (luminance > 0.15 && greenDominance < 0.15 && 
                                            (color.r > 0.3 || color.b > 0.3)) {
                                            // Calculate flower glow based on distance from center
                                            float centerDistance = length(signMidCoordPos);
                                            float glowRadius = 0.9; // Larger glow radius for potted flowers
                                            float glowFalloff = max(0.0, 1.0 - centerDistance / glowRadius);
                                            emission = luminance * 2.8 * (0.5 + 0.5 * glowFalloff);
                                            color.rgb *= 1.15 + 0.25 * glowFalloff;
                                        }
                                    }
                                }
                            }

...
// Modified by Haider
    ...
    const float cloudStretchRaw = 25.0;
#elif CLOUD_QUALITY == 2
    const float cloudStretchRaw = 35.0;
#elif CLOUD_QUALITY == 3
    const float cloudStretchRaw = 40.0;
#endif
...
const float cloudNarrowness = 0.00005;
...
float GetCloudNoise(vec3 tracePos, int cloudAltitude, float lTracePosXZ, float cloudPlayerPosY) {
    ...
    #define CLOUD_BASE_ADD 1.5
    //#define CLOUD_FAR_ADD -0.005
    #define CLOUD_ABOVE_ADD 0.3
    ...
    return noise - (threshold * 0.1 + 0.15);
}
